...
mod tests {
    ...
    impl resolve::ClassLoader for FakeClassLoader {
        fn try_load(&self, name: &str, _: &mut dyn FnMut (&str) -> Result<ClassId, resolve::ClassResolveError>) -> Option<Result<ResolvedClass, resolve::ClassResolveError>> {
            ...
            } else if name == "java/lang/Object" || name == "java/lang/invoke/MethodHandle" {
                Some(Result::Ok(ResolvedClass::User(create_mock_class(&[]))))
            } else {
                None
            }
    ...
    fn create_fake_env() -> ClassEnvironment {
        let mut env = ClassEnvironment::new(vec![
            Box::new(resolve::ArrayClassLoader()),
        ...
        compute_all_layouts(&mut env, &liveness, false);

        env
    }

    lazy_static! {
        static ref TEST_ENV: ClassEnvironment = create_fake_env();
    }
